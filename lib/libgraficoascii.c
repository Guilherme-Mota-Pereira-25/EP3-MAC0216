...
void escreve_specs(FILE* specs, int linhas, char *nomes_linhas[1000]) {
    ...
    fprintf(specs, "set term dumb 130 50\n");
    fprintf(specs, "set datafile missing \'-1.000000\'\n");
    fprintf(specs, "set key outside\n");



    char titulo_rotulos[200];
    strcpy(titulo_rotulos, titulo_grafico);
    strcat(titulo_rotulos, " (Eixo x: ");
    strcat(titulo_rotulos, rotulo_x);
    strcat(titulo_rotulos, ", Eixo y: ");
    strcat(titulo_rotulos, rotulo_y);
    strcat(titulo_rotulos, ")");
    escreve_parametro(specs, "title", titulo_rotulos);
